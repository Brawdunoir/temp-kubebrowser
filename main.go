...
import (
	...
	kubeinformers "github.com/brawdunoir/kubebrowser/pkg/client/informers/externalversions"
)
...
func main() {
	...
	kubeInformerFactory := kubeinformers.NewSharedInformerFactory(exampleClient, time.Second*30)

	lister := kubeInformerFactory.Kubeconfig().V1().Kubeconfigs().Lister()

	kubeconfigs, err := lister.List(nil)
	if err != nil {
		logger.Error(err, "Error listing kubeconfigs")
		klog.FlushAndExit(klog.ExitFlushTimeout, 1)
	}
	for _, kubeconfig := range kubeconfigs {
		logger.Info("Kubeconfig: ", kubeconfig.Name)
	}
}
